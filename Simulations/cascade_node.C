...
int main() {
	...
	cout << "5 - Watts-Strogatz Model w/ <k> = 4" << endl;

	int model;
	cin >> model;
	while(cin.fail() || model < 0 || model > 5) {
	...
	if (model == 0) {
		...
		strcpy(file_name, "Data/WS/data_0.txt");
		file_name[13] = criterion + '0';
	}
	...
	for(int n = 0; n < N_GRAPHS; ++n) {
		...
		if (model == 0) {
			...
			char filename[50] = {0};
			sprintf(filename, "Simulations/Graphs/ws_%d.gml", n);
