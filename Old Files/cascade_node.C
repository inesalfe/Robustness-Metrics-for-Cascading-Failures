...
int main() {
	...
	cout << "3 - Barabasi Albert Model w/ M=1" << endl;
	...
	if (model == 0) {
		...
		strcpy(file_name, "Data/BA_M1/data_0.txt");
		file_name[16] = criterion + '0';
	}
	...
	for(int n = 0; n < N_GRAPHS; ++n) {
		...
		if (model == 0) {
			...
			char filename[50] = {0};
			sprintf(filename, "Simulations/Graphs/ba_m1_%d.gml", n);
